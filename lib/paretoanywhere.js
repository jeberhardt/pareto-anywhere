...
function initialiseExpressMiddleware (app, router, instance, options) {
  ...
  if (!Object.prototype.hasOwnProperty.call(options, 'useCors')) {
...
function createBarnowl (app, server, options) {
  if (!Object.prototype.hasOwnProperty.call(options, 'barnowl')) {
    options.barnowl = { enableMixing: true }
  }

  const barnowl = new Barnowl(options.barnowl)
...
function createBarnacles (barnowl, io, options) {
  if (!Object.prototype.hasOwnProperty.call(options, 'barnacles')) {
    options.barnacles = {}
  }
  ...
  const barnacles = new Barnacles(options.barnacles)

  barnacles.addInterface(BarnaclesSocketIO, { io })
...
function createBarterer (app, barnacles, chimps, io, options) {
  if (!Object.prototype.hasOwnProperty.call(options, 'barterer')) {
    options.barterer = {}
  }
  ...
  const barterer = new Barterer(options.barterer)
...
function createChickadee (app, barnacles, chimps, io, options) {
  if (!Object.prototype.hasOwnProperty.call(options, 'chickadee')) {
    options.chickadee = {}
  }
  ...
  const chickadee = new Chickadee(options.chickadee)
...
function createChimps (barnowl, barnacles, io, options) {
  if (!Object.prototype.hasOwnProperty.call(options, 'chimps')) {
    options.chimps = {}
  }
  options.chimps.barnowl = barnowl
  options.chimps.barnacles = barnacles

  const chimps = new Chimps(options.chimps)

  // TODO: share barnowl-socketio instance with barnacles
  chimps.addInterface(BarnaclesSocketIO, { io })
...
function createJsonSilo (app, options) {
  if (!Object.prototype.hasOwnProperty.call(options, 'jsonsilo')) {
    options.jsonsilo = {}
  }
  options.jsonsilo.app = app

  const jsonsilo = new JSONSilo(options.jsonsilo)
...
function createPacketProcessors () {
  fs.readFile(path.resolve(path.join(__dirname, '..', 'config', 'enocean.json')),
    (err, data) => {
      if (!err) {
        try {
